...
const AdminSwiss = () => {
  ...
  const [teamSwissStatuses, setTeamSwissStatuses] = useState<Record<string, string | null>>({});
  ...
  const fetchData = async () => {
    try {
      const [teamsRes, matchesRes] = await Promise.all([
        supabase.from('teams').select('*').in('status', ['registered', 'ready']),
      ...
      // Build swiss status map
      const statusMap: Record<string, string | null> = {};
      teamsData.forEach((t: any) => { statusMap[t.id] = t.swiss_status || null; });
      setTeamSwissStatuses(statusMap);
        ...
        <TabsContent value="standings" className="space-y-6">
          <div className="glass-card p-6">
            ...
            <table className="w-full">
              ...
              <tbody>
                {standings.map((s, i) => {
                  const swissStatus = s.team_id ? teamSwissStatuses[s.team_id] : null;
                  const advanced = swissStatus === 'advanced';
                  const eliminated = swissStatus === 'eliminated';
